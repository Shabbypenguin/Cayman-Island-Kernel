...
//
//static	bool	gotFifoUnderRunOverRun = FALSE; // kenneth.kang@lge.com We did not use this
...
//
//static	bool	deglitchingRsenNow = FALSE; // kenneth.kang@lge.com We did not use this
...
//static	void 	DeglitchRsenLow( void ); // kenneth.kang@lge.com We did not use this function
...
// SANGWOO followings have to redefine. // kenneth.kang@lge.com We did not use this
...
//extern void rcp_cbus_uevent(u8);		//NAGSM_Android_SEL_Kernel_Aakash_20101206 R1
...
Bool SiI9234_init(void)
{
	...
	SiI9234_HW_Reset();	
	SiiMhlTxInitialize();
//	SET_BIT(0x72, 0x79, BIT_4 | BIT_5);
//	msleep(10);
	printk("ksw CLR_BIT : Read(0x72, 0x79) = 0x%x\n", I2C_ReadByte(0x72, 0x79));
	return TRUE;
}
...
/*
	...
	msleep(10);
*/
	printk("ksw %s : READ(0x72, 0x79) = 0x%x\n", __func__, I2C_ReadByte(0x72, 0x79));
...
void	SiiMhlTxDrvTmdsControl( bool enable )
{
	if( enable )
	{
		printk("1: %s I2C_ReadByte(0x72, 0x80) = 0x%x\n", __func__, I2C_ReadByte(0x72, 0x80));
		SET_BIT(SA_TX_Page0_Primary, 0x80, 4);
		printk("2: %s I2C_ReadByte(0x72, 0x80) = 0x%x\n", __func__, I2C_ReadByte(0x72, 0x80));
...
//
#if 0 // kenneth.kang@lge.com We did not use this function
 ...
 void WriteInitialRegisterValues ( void )
{
	...
	//ReadModifyWriteTPI(0x79, BIT_2, 0); //Set interrupt active low HML kenneth.kang
...
void	ProcessRgnd( void )
{
	...
	{
		TX_DEBUG_PRINT((" %d: USB impedance. Disable MHL discovery.\n", (int)reg99RGNDRange));
		//3355
		if(vbus_mhl_est_fail == TRUE)	
		{
			MhlTxDrvProcessDisconnection();
	...
	else 	
	{
		...
		else if(0x02==reg99RGNDRange)
		{
			...
			// Discovery enabled
			SET_BIT(0x72, 0x79, 4);
			SET_BIT(0x72, 0x79, 5); // kenneth.kang@lge.com change HPD status low to high
...
void	SwitchToD0( void )
{
	...
	ReadModifyWriteTPI(0x79, BIT_5 | BIT_4, BIT_4);
	msleep(300);
	printk("1: I2C_ReadByte(SA_TX_Page0_Primary, (0x74)); = 0x%x\n", I2C_ReadByte(SA_TX_Page0_Primary, (0x74)));
	printk("1: GPIO_HPD : 0x%x\n",gpio_get_value(172));
	printk("1: %s : REG81 : 0x%x\n", __func__, I2C_ReadByte(0x72, 0x81));
	ReadModifyWriteTPI(0x79, BIT_5 | BIT_4, BIT_5 | BIT_4);
	msleep(300);
	printk("2: I2C_ReadByte(SA_TX_Page0_Primary, (0x74)); = 0x%x\n", I2C_ReadByte(SA_TX_Page0_Primary, (0x74)));
	printk("2: GPIO_HPD : 0x%x\n",gpio_get_value(172));
	printk("2: %s : REG81 : 0x%x\n", __func__, I2C_ReadByte(0x72, 0x81));
	*/
	// Setup interrupt masks for all those we are interested.
	MASK_INTR_1_INTERRUPTS;
...
/*
	printk("ksw HPD = %d\n", gpio_get_value(172));
	SET_BIT(0x72, 0x79, BIT_4 | BIT_5);
	//CLR_BIT(0x72,0x79, BIT_4 | BIT_5);
	msleep(10);
	printk("ksw HPD = %d, I2C_READ(0x72,0x79) = 0x%x\n", gpio_get_value(172), I2C_ReadByte(0x72, 0x79));
*/	
			...
			case	MHL_MSC_MSG_RCP:
				...
				if(MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData] )
				{
					...
					*eventParameter = mhlTxConfig.mscMsgData; // key code
			        //rcp_cbus_uevent(*eventParameter);	//MHL v1 //NAGSM_Android_SEL_Kernel_Aakash_20101126 //NAGSM_Android_SEL_Kernel_Aakash_20101206 R1
