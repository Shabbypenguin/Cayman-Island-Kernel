...
DECLARE_COMPLETION(wait_resume);
#endif
...
exit_AMI306_WIA:
	if (res <= 0) {
		if (printk_ratelimit()) {
			AMIE("I2C error: AMI306_WIA ret value=%d\n", res);
		}
		return -3;
	}
...
static int AMI306_ReadSensorData2(short *buf)
{
	char cmd;
	int mode = 0;
	unsigned char databuf[10] = {0,};
...
exit_AMI306_ReadSensorData:
	if (res <= 0) {
		if (printk_ratelimit()) {
			AMIE("AMI306_ReadSensorData2: I2C error, ret value=%d\n", res);
		}
		return -3;
	}
		report_cnt ++;
...
static int ami306_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
	char strbuf[AMI306_BUFSIZE];
	int controlbuf[AMI306_CB_LENGTH];
	int valuebuf[4];
...
static int ami306daemon_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
	int valuebuf[4];
#if defined(STM_GYRO)	//20110630
...
static int ami306hal_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
	int controlbuf[AMI306_CB_LENGTH];
	char strbuf[AMI306_BUFSIZE];
	int pedoparam[AMI306_PD_LENGTH];		
...
static int __init ami306_init(void)
{
	...
	if (res) {
		AMIE("ami306_init, failed to probe i2c, line(%d) \n", __LINE__);
