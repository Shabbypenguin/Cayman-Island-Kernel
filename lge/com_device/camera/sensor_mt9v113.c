...
/*
	rc = mt9v113_i2c_read(mt9v113_client->addr,
		0x0018, &poll_reg_1, WORD_LEN);

	if (rc<0)
		return rc;
	else
		CDBG("POLL_REG 0x0018 ==>0x%x", poll_reg_1);
	rc = mt9v113_i2c_read(mt9v113_client->addr,
		0x301A, &poll_reg_2, WORD_LEN);

	if (rc<0)
		return rc;
	else
		CDBG("POLL_REG 0x301A ==>0x%x", poll_reg_2);
*/
#endif
...
/*
static int mt9v113_set_antibanding(int mode)
{
	int32_t rc = 0;
	...
	switch (mode) {
		case CAMERA_ANTIBANDING_60HZ:
			rc = mt9v113_i2c_write_w_sensor(0x098E, 0xCC03);
			if (rc < 0)
				return rc;	
			rc = mt9v113_i2c_write_w_sensor(0x0990, 0x0200);
			if (rc < 0)
				return rc;
			break;
		case CAMERA_ANTIBANDING_50HZ:
			...
			if (rc < 0)
				return rc;
			rc = mt9v113_i2c_write_w_sensor(0x0100, 0x0100);
			if (rc < 0)
				return rc;
			break;
		default:
...
static int brightness_table[] = {0x0012, 0x001A, 0x0022, 0x002A, 0x0032, 0x003A, 0x0042,  0x004A, 0x0052, 0x005A, 0x0062, 0x006A, 0x0072};
...
static int mt9v113_reg_preview(void)
{
	...
	msleep(150); //miracle.kim@lge.com 2011-07-21 added for stability

/* 	rc = mt9v113_i2c_write_table(mt9v113_regs.prev_tbl, mt9v113_regs.prevtbl_size);

	if (rc<0)
	{
		CDBG("###[ERROR]%s: failed to write preview table\n", __func__);
		return rc;
	}
...
*/ //miracle.kim@lge.com 2011-07-21 removed for reducing entry time
	return rc;
}
...
/* 	rc = mt9v113_i2c_write_table(mt9v113_regs.snap_tbl, mt9v113_regs.snaptbl_size);
	if (rc < 0)
		return rc;
