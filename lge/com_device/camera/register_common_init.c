...
#define CAMSENSOR_SUB_PARAM_FILE  PARAM_ROOT"sensorparamtable_sub.txt"
...
LOCAL byte* open_param_file(void)
{
  ...
  set_fs(KERNEL_DS);
	...
	flip = filp_open(CAMSENSOR_MAIN_PARAM_FILE, O_RDONLY |O_LARGEFILE, S_IRUSR);
	if (IS_ERR(flip)) {
		printk("sensor init file open error!\n");
		goto err_open;
	}
