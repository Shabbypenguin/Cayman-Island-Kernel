...
void lm3559_enable_torch_mode(int state)
{
    if(state == LM3559_LED_LOW){
		...
		/* 111 : 112.5 mA  */ 
		lm3559_i2c_write(lm3559_client,LM3559_REG_TORCH_BRIGHTNESS,0x1B);
	}
...
void lm3559_power_onoff(int onoff){
	...
	if(onoff == LM3559_POWER_OFF)
		gpio_set_value(lm3559_pdata->gpio_flash_en, 0);		
	else
		gpio_set_value(lm3559_pdata->gpio_flash_en, 1);		
...
static int __devinit lm3559_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gpio_request(lm3559_pdata->gpio_flash_en, "cam_flash_en");

	gpio_tlmm_config(GPIO_CFG(lm3559_pdata->gpio_flash_en, 0, GPIO_CFG_OUTPUT,
