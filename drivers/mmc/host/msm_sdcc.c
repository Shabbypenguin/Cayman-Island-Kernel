...
/* byongdoo.oh@lge.com remove error message at pulling sd card without unmount */
	...
	if (host->curr.got_dataend || mrq->data->error) {
		 ...
		 */
		if (host->curr.got_dataend && !(readl_relaxed(host->base + MMCIMASK0) & MCI_DATAENDMASK)) {
			msmsdcc_reset_and_restore(host);
			writel_relaxed((readl_relaxed(host->base + MMCIMASK0) | MCI_DATAENDMASK), host->base + MMCIMASK0);
			msmsdcc_delay(host);
		}
...
// when the card is removed while writing, rarely null pointer panic occurs
...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if ((loop_cnt >= 5) && ret == 1) {
			pr_info("%s: Stuck in IRQ handler force exit,"
					"status=0x%.8x\n", __func__,
					readl_relaxed(host->base + MMCISTATUS));
			if (host->curr.got_dataend &&
				(readl_relaxed(host->base + MMCISTATUS)	& MCI_DATAEND)) {

				if (host->dma.busy) {
					writel_relaxed((readl_relaxed(host->base + MMCIMASK0)
						& ~MCI_DATAENDMASK), host->base + MMCIMASK0);
					msmsdcc_delay(host);
				} else
					msmsdcc_reset_and_restore(host);
			}
...
//ADD_START, secheol.pyo@lge.com, 2011-09-01, [MDM BSP] insert debugging code from QCT request (case: 609353)
             {
                    int i;
...
//ADD_END, secheol.pyo@lge.com, 2011-09-01, [MDM BSP] insert debugging code from QCT request (case: 609353)				
