...
extern int lm3559_flash_set_led_state(unsigned state);
#endif
...
int msm_camera_flash_lm3559(unsigned led_state)
{
	...
	CDBG("msm_camera_flash_lm3559: led_state = %d, return %d\n",led_state ,rc);
	...
	struct msm_camera_sensor_flash_data *fdata, unsigned led_state)
{
	...
	case MSM_CAMERA_FLASH_SRC_CURRENT_DRIVER:
		...
		break;
#ifdef CONFIG_LGE_FLASH_LM3559
	case MSM_CAMERA_FLASH_SRC_LM3559:
...
{
	int rc = 0;
	CDBG("msm_strobe_flash_ctrl: strobe_ctrl->type =  %d\n", strobe_ctrl->type);
