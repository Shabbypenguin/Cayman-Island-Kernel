...
int acm_bind_config(struct usb_configuration *c, u8 port_num)
{
	...
	if (acm_string_defs[ACM_CTRL_IDX].id == 0) {
		...
		if (status < 0)
			return status;
// [START] seunghun.kim : for LG_USB_DRIVER 2011.03.25
#if defined(CONFIG_LGE_USB_GADGET_NDIS_VZW_DRIVER) || defined(CONFIG_LGE_USB_GADGET_NDIS_UNITED_DRIVER)
		acm_string_defs[ACM_DATA_IDX].id = 7;
		acm_data_interface_desc.iInterface = 7;
#else
// [END] seunghun.kim : for LG_USB_DRIVER 2011.03.25
