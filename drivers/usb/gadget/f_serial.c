...
//			__func__, no_tty_ports, no_sdio_ports, nr_ports);
	...
	pr_info("daniel.kang %s (%d): no_tty_ports:%u no_sdio_ports:%u no_smd_ports:%u nr_ports:%u\n",
			__func__, ret, no_tty_ports, no_sdio_ports, no_smd_ports, nr_ports);
	return ret;
}
#endif
static int gport_connect(struct f_gser *gser)
{
	...
	pr_info("daniel.kang %s: transport:%s f_gser:%p gserial:%p port_num:%d\n",
...
static int gport_disconnect(struct f_gser *gser)
{
	...
	pr_debug("daniel.kang %s: transport:%s f_gser:%p gserial:%p port_num:%d\n",
			__func__, transport_to_str(gser->transport),
			gser, &gser->port, gser->port_num);
...
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
	if (gser_string_defs[ACM_CTRL_IDX].id == 0) {
		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		pr_info("%s: serial ctrl status %d\n", __func__, status);
		gser_string_defs[ACM_CTRL_IDX].id = status;
		gser_control_interface_desc.iInterface = status;
		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		pr_info("%s: serial data status %d\n", __func__, status);
		gser_string_defs[ACM_DATA_IDX].id = 7; //status;
		gser_data_interface_desc.iInterface = 7; //status;
	}
#else
