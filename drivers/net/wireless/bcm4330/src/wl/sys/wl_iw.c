...
// LGE_VERIZON_WIFI_S, [dongseok.ok@lge.com], 2011.07.08 <Verizon MobileHotSpot>
#if defined(CONFIG_LGE_BCM432X_PATCH)
static int wl_iw_softap_deassoc_sta_list(struct net_device *dev, void* maclist);
static int wl_iw_softap_ap_isolate(struct net_device *dev, void* isIsolate);
#endif //CONFIG_LGE_BCM432X_PATCH
// LGE_VERIZON_WIFI_E, [dongseok.ok@lge.com], 2011.07.08 <Verizon MobileHotSpot>
#endif /* SOFTAP */
...
//LGE_VERIZON_WIFI_E, [jeongah.min@lge.com], 2011-07-21,  WiFi  wifi 5GHz update & tunning 
...
__u16 *merged_len)
{
	...
	for (;node;) {
/* LGE_VERIZON_WIFI_S, [jeongah.min@lge.com], 2011-07-22,  Wifi driver update*/ 
#ifdef CONFIG_LGE_BCM432X_PATCH	
		list_merge = (wl_scan_results_t *)&node->buflen;
#else	
		list_merge = (wl_scan_results_t *)node;
#endif /* CONFIG_LGE_BCM432X_PATCH */
/* LGE_VERIZON_WIFI_E, [jeongah.min@lge.com], 2011-07-22,  Wifi driver update*/ 	
...
//LGE_VERIZON_WIFI_S, [kyubyoung.lee@lge.com], 2011.08.18, changed it to 80ms(chip default value)<- scan time(50ms) per 1-ch wifi 5GHz update & tunning 
	//bill.jung@lge.com
	if( g_modify_scan_time != 0 )
	{
		...
		if( g_modify_scan_time > 2 )
		{
			...
			dev_wlc_intvar_set(dev, "scan_unassoc_time", 80/*50*/);
		
			g_modify_scan_time = 0;
		}
	}
//LGE_VERIZON_WIFI_E, [kyubyoung.lee@lge.com], 2011.08.18,  WiFi  wifi 5GHz update & tunning 
...
)
{
	...
	wl_uint32_list_t *list;
	int nchan;
	char buf[WLC_CNTRY_BUF_SZ +1];
	int error;
	char *p = extra;
  // LGE_VERIZON_WIFI_S, [kyubyoung.lee@lge.com], 2011.09.02, fixed a problem that it doesn't be set it when user set channel 13
#if defined(CONFIG_LGE_BCM432X_PATCH)
...
// For specific country style. ex) AU/2
	...
	if (revstr)
	{
		...
		memcpy(cspec.country_abbrev, buf, WLC_CNTRY_BUF_SZ);
		cspec.country_abbrev[2] = '\0';
		memcpy(cspec.ccode, cspec.country_abbrev, WLC_CNTRY_BUF_SZ);
		memset(smbuf, 0, sizeof(smbuf));
//printf("P:config country code is country : %s, rev : %d !!\n", cspec.country_abbrev, cspec.rev);
		iolen = bcm_mkiovar("country", (char *)&cspec, sizeof(cspec), smbuf, sizeof(smbuf));
		error = dev_wlc_ioctl(dev, WLC_SET_VAR, smbuf, iolen);
	}
	else
#endif
...
//printk("P:nchan[%d]:error[%d],buf[%s]:sizeof(buf)[%d]\n", nchan, error, buf, sizeof(buf));
...
#if defined(CONFIG_LGE_BCM432X_PATCH) && defined(SOFTAP)
static int
wl_iw_softap_deassoc_sta_list(struct net_device *dev, void* buf)
{
	int i;
	int res = 0;
    struct maclist *deassoc_maclist = (struct maclist *)buf;

	if (!ap_net_dev) {
		WL_ERROR(("%s: ap_net_dev is null !!!", __FUNCTION__));
		return -1;
    }
    dev = ap_net_dev;

	if (deassoc_maclist->count)
    {
        for( i = 0; i < deassoc_maclist->count; i++ )
        {
            scb_val_t scbval;
            scbval.val = htod32(1);
                    
            bcopy(&deassoc_maclist->ea[i], &scbval.ea, ETHER_ADDR_LEN);

            res |= dev_wlc_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON,
                                 &scbval, sizeof(scb_val_t));

            WL_SOFTAP(("%s:deauth STA[%d] - res(%d) - " \
    ...
    {
		WL_SOFTAP((" STA DEASSOC list is empty\n"));
    }

	if (res != 0)
		WL_SOFTAP((" Error:%d in :%s\n", res, __FUNCTION__));
	else if (deassoc_maclist->count) {
...
static int
wl_iw_softap_ap_isolate(struct net_device *dev, void* buf)
{
	int res = 0;
	unsigned int isIsolate = 0;
    
	isIsolate = *(unsigned int*)buf;

	WL_ERROR(("%s: isIsolate (%d)", __FUNCTION__, isIsolate));
	...
	{
		WL_ERROR(("%s:%d> isIsolate (%d) is out of bound ( 0 or 1 )\n", \
			__FUNCTION__, __LINE__, isIsolate ));
		return -1;
	}
...
#endif
	if (!dev) {
		WL_ERROR(("%s: dev is null !!!", __FUNCTION__));
		return -1;
	}
	
	//set
	res = dev_wlc_intvar_set(dev, "ap_isolate", isIsolate);
...
// LGE_VERIZON_WIFI_S, [dongseok.ok@lge.com], 2011.07.08 <Verizon MobileHotSpot>
#if defined(CONFIG_LGE_BCM432X_PATCH)
		else if (strnicmp(extra, "AP_SET_DROP_STA", strlen("AP_SET_DROP_STA")) == 0) {
			WL_SOFTAP(("%s: set AP_SET_DROP_STA\n", __FUNCTION__));
			wl_iw_softap_deassoc_sta_list(dev, (extra + PROFILE_OFFSET));
		}
		else if (strnicmp(extra, "AP_SET_ISOLATE", strlen("AP_SET_ISOLATE")) == 0) {
			WL_SOFTAP(("%s: set AP_SET_ISOLATE\n", __FUNCTION__));
#if 0				/* For test */
			do {
				int i = 0;
				printk("%s:%d, extra=", __FUNCTION__, __LINE__);
				for (i = 0; i < 64; i++) {
					if (i%16 == 0)
						printk("\n[%02d]", i);
					else if (i%4 == 0)
...
#endif
			wl_iw_softap_ap_isolate(dev, (extra + PROFILE_OFFSET));
	    }
#endif /* CONFIG_LGE_BCM432X_PATCH */
// LGE_VERIZON_WIFI_E, [dongseok.ok@lge.com], 2011.07.08 <Verizon MobileHotSpot>
#endif /* SOFTAP */
