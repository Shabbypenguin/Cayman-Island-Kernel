...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
	...
	case EXTERNAL_ENABLE 	: INC_READY(ucI2CID,	175280); break;
...
#ifdef INC_SCAN_FAST_WITH_RF_HIGH_LEVEL_ENABLE
INC_UINT8 INC_PHY_RESET(INC_UINT8 ucI2CID)
{
	...
	for(nLoop = 0; nLoop < 10; nLoop++){
		...
		int uStatus = INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x3B) & 0xFFFF;
		if(uStatus == 0x8000) 
			break;
	}
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
	...
	while(1)
	{
		...
		// Sync Reset 관련(Resync Counter를통한Reset, CF는lock이지만CT Unlock)
		// 7번연속으로4에머무를경우Reset 수행, 3회이상발생하면없는channel로간주.
		...
		// Coarse Frequency는Lock이지만, Coarse Time을UnLock일경우Reset 수행.
		...
		// 강전계예외처리
		// wOperation은4까지증가하지만FFT Spectrum Power는증가하지않는경우
		else if( wOperState > 0x2 && ((awData[0]&0x0F00) == 0) ) {
			pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL;
			break;
		}
		// Coarse Time 예외처리0x2000 이하일경우강전계폴더신호.
...
INC_UINT8 INC_ENSEMBLE_SCAN(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
	...
	printk("INC_ENSEMBLE_SCAN ulFreq = (%d)\n", ulFreq);
...
INC_UINT8 INC_STATUS_CHECK(INC_UINT8 ucI2CID)
{
	...
	if(pInfo->IsReSync == 1 && pInfo->ucCERCnt == 0)
	{
		INC_MSG_PRINTF(1, " \r\n Resync Escaped!!!!!!!! [CER :%d]===> \r\n\r\n", pInfo->uiCER);
		pInfo->IsReSync = 0;
		INC_AIRPLAY_SETTING(ucI2CID);
	}
