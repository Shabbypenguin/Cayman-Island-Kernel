...
static int bluetooth_power_rfkill_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "[LG_BTUI] %s : Called bluetooth_power_rfkill_probe",__func__);  	 	
/* LGE_CHANGES_S [taekeun1.kim@lge.com] 2010-06-06, for bt */
#if defined (CONFIG_MACH_LGE)
...
static int __devinit bt_power_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "%s\n", __func__);
	printk(KERN_ERR "[LG_BTUI] %s : Called bt_power_probe",__func__);  	 	
...
{
	int ret;
	printk(KERN_ERR "[LG_BTUI] %s : Called bluetooth_power_init",__func__);  	 	
