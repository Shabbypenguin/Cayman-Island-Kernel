...
static int voice_config_cvs_vocoder(struct voice_data *v)
{
	...
	case VSS_MEDIA_ID_EVRC_MODEM: {
		...
		pr_info("%s: Setting  min-max rate of type %d\n",
			__func__, v->mvs_info.media_type);
		...
		cvs_set_cdma_rate.cdma_rate.min_rate =
				v->mvs_info.min_max_rate.min_rate;
		cvs_set_cdma_rate.cdma_rate.max_rate =
				v->mvs_info.min_max_rate.max_rate;

		v->cvs_state = CMD_STATUS_FAIL;

		ret = apr_send_pkt(apr_cvs, (uint32_t *) &cvs_set_cdma_rate);
		if (ret < 0) {
			pr_err("%s: Error %d sending SET_ENC_MINMAX_RATE\n",
			       __func__, ret);
		...
		// please add follow lines
        if ( (v->mvs_info.media_type == VSS_MEDIA_ID_4GV_NB_MODEM ) || (v->mvs_info.media_type == VSS_MEDIA_ID_4GV_WB_MODEM) )
        {
			ret = voice_set_dtx(v);
        }
	...
	case VSS_MEDIA_ID_AMR_WB_MODEM: {
		...
		if ((v->mvs_info.media_type == VSS_MEDIA_ID_4GV_NB_MODEM) ||
		(v->mvs_info.media_type == VSS_MEDIA_ID_4GV_WB_MODEM))
			ret = voice_set_dtx(v);
			  ...
			  struct q_min_max_rate min_max_rate)
{
	...
	voice.mvs_info.min_max_rate = min_max_rate;
}
